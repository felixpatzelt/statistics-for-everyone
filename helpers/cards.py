...
def plot_first_experiment_bars(data, n_card_pairs):
    ...
    input_n_cards = alt.binding_range(
        min=1,
        max=n_card_pairs,
        step=1,
        name="Draw Card Pairs: ",
    )
    selection = alt.selection_single(bind=input_n_cards, init={"card_pair": 1})
    ...
    return (
        ...
        .transform_filter("datum.experiment == 1")
...
def plot_first_experiment_lines(data):
    ...
    ticks = (
        ...
        .transform_filter("(datum.experiment == 1) & (datum.win > 0)")
        .encode(
            alt.X(
                "card_pair:Q",
                title="Cards drawn per stack",
                axis=alt.Axis(
                    ticks=False, grid=False, labels=False, title="Wins", orient="top"
                ),
            ),
            alt.Y("stack:Q", title="Stack", scale=alt.Scale(domain=[2, 0]), axis=None),
            color=alt.Color("stack:N", title="Stack"),
        )
        .properties(width=plot_width, height=35, view=alt.ViewConfig(strokeWidth=0))
    )
    ...
    lines = (
        ...
        .mark_line()
        .transform_window(
            cumulative_wins="sum(win)", frame=[None, 0], groupby=["experiment", "stack"]
        )
        .transform_filter("datum.experiment == 1")
        .encode(
        ...
        .properties(width=plot_width, height=265)
    )
...
def plot_repeated_experiments(
    data, n_card_pairs, show="Experiments, Histogram, Std", plot_height=225
):
    plots = []
    ...
    selection = alt.selection_single(bind=input_n_cards, init={"card_pair": 25})

    # filter data
    base = (
        alt.Chart(data)
        .add_selection(selection)
        .transform_filter(alt.datum.card_pair <= alt.expr.toNumber(selection.card_pair))
        .transform_aggregate(p_win="mean(win):Q", groupby=["stack", "experiment"])
    )
        ...
        dots = (
            ...
            .encode(
                ...
                y=alt.Y(
                    "p_win:Q",
                    scale=scale,
                    axis=alt.Axis(values=np.arange(0, 1.1, 0.2), grid=True),
                    title="Winning probability",
                ),
                color="stack:N",
            )
            .properties(width=400, height=plot_height)
        )
        plots.append(dots)
    ...
    if "histogram" in show.lower():
        ...
        hist = (
            ...
            .properties(width=150, height=plot_height)
        )
        plots.append(hist)
    ...
    if "std" in show.lower():
        ...
        std_text = (
            alt.Chart(
                pd.DataFrame(
                    {"x": [4], "y": [0.5], "text": ["Mean ± Standard Deviation"]}
                )
            )
            .mark_text(angle=90, baseline="middle")
        ...
        ).properties(width=25, height=plot_height)
        plots.append(errorbars)

    # combine & show
    alt.concat(*plots).configure_axis(grid=False).configure_view(strokeWidth=0).display(
        renderer="svg"
    )
...
def plot_experiment_bars_with_errors(data, n_card_pairs, n_card_pairs_init, n_repeats):
    ...
    selection_n_cards = alt.selection_single(
        bind=input_n_cards, init={"card_pair": n_card_pairs_init}
    )
    input_experiment = alt.binding_range(
        min=1, max=n_repeats, step=1, name="Select one experiment: "
    )
    selection_experiment = alt.selection_single(
        bind=input_experiment, init={"experiment": 1}
    )

    # filter data
    base = (
        alt.Chart(data)
        .add_selection(selection_n_cards, selection_experiment)
        .transform_filter(
            # there are sometimes problems with automatic types in comparisons,
            # when comparing datum with a selection.
            # hence we just put the explicit types everywhere
            (
                alt.expr.toNumber(alt.datum.card_pair)
                <= alt.expr.toNumber(selection_n_cards.card_pair)
            )
            & (
                alt.expr.toNumber(alt.datum.experiment)
                == alt.expr.toNumber(selection_experiment.experiment)
            )
        )
    )
    ...
    bars = base.mark_bar().encode(
        x="stack:N", y="mean(win):Q", color=alt.Color("stack:N", legend=None)
    )
    ...
    # combine plot
    alt.layer(bars, errors).properties(width=250, height=250).configure_axis(
        grid=False
    ).configure_view(strokeWidth=0).display(renderer="svg")
